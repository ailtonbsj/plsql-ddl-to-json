...
export async function plsqlToJs(fileName: string) {
    ...
    properties.map(property => {
        ...
        if (columnDefinition !== null) {
            const col = { key: false, nullable: true };
            ...
            if (inline !== null && inline.length > 0) {
                ...
                if (check !== null) {
                    ...
                    const compoundExpr = unary.multiset_expression().relational_expression().compound_expression();
                    if (compoundExpr !== null) {
                        const colName: string = compoundExpr.concatenation(0).getText();
                        const inElements = compoundExpr.in_elements();
                        if (inElements !== null) {
                            const list: string[] = inElements.concatenation_list().map(i => i.getText().replaceAll("'",''));
                            sqlObj.columns[colName.toUpperCase()].enumItems = list;
                        }
                    }
                }
